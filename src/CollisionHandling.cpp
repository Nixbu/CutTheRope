...
{
     auto& candy = dynamic_cast<Candy&>(object1);
     auto& bubble = dynamic_cast<Bubble&>(object2);
...
{
    auto& candy = dynamic_cast<Candy&>(object1);
    auto& omnom = dynamic_cast<Omnom&>(object2);
...
{
    auto& candy = dynamic_cast<Candy&>(object1);
    auto& star = dynamic_cast<Star&>(object2);
...
{
    auto& candy = dynamic_cast<Candy&>(object1);
    auto& spike = dynamic_cast<Spikes&>(object2);
...
{
    auto& candy = dynamic_cast<Candy&>(object1);
    auto& doubleHat = dynamic_cast<DoubleHat&>(object2);
...
{
    // Attempt to cast to Candy and Air types using std::dynamic_cast
    try {
        auto& candy = dynamic_cast<Candy&>(object1);
        auto& air = dynamic_cast<Air&>(object2);

        // If we reach this point, the casts were successful
        candy.getBody()->ApplyForceToCenter(air.getForce(), true);
    }
    catch (const std::bad_cast& e) {
        // Handle invalid object types
        throw std::runtime_error("Invalid object types for candyAir function: " + std::string(e.what()));
    }
